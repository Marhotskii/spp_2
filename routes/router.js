...
router.post("/api/register", (req, res) => {
    ...
    else {res.status(405).json({error: "User already exists"})}
});
...
router.post("/tasks", verifyToken, (req, res) => {
    ...
    if (filedata) { filepath = path.join("uploads/", filedata.filename);}   
...
router.put("/tasks", verifyToken, (req, res) => {
    ...
    if (body.completed != undefined) {task.completed = true;} 
    else {task.completed = false;}
    if (filepath != undefined) {task.filename = filepath;}
    let deleteFile = body.deleteFile == "true";
    if (deleteFile) {task.filename = undefined;}
